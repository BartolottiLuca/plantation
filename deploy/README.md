...
| Image tag | `image.tag` | Written by C14's CI write-back after each push to `main`; never `latest` |
...
- `image.tag` is a plain scalar so CI can rewrite it with `yq` (C14) without a
